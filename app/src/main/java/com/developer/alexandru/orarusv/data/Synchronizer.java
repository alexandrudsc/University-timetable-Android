...
/**
 * Created by Alexandru on 12/23/2014. Fetches the faculties from the URL and their IDs storing in a
 * local database
 */
@Deprecated
public class Synchronizer extends IntentService {
  ...
  String generalName =
      ""; // Some faculties have a general prefixed name (like the "Pedagogic department")
  ...
  void addGroupToDatabase(String[] data) {
    String name;
    if (data.length > 6) name = data[3] + " an " + data[4] + "__" + data[5] + data[6];
    else name = data[3] + " an " + data[4] + "__" + data[5];
    Log.d(TAG, name);
  }
