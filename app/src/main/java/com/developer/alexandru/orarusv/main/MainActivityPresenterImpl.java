...
/** Created by alexandru on 10/25/16. The implementation of Presenter for MainActivity */
public class MainActivityPresenterImpl implements MainActivityPresenter {
  ...
  public boolean onCourseClicked(Course course) {
    ...
    if (courseFragment != null) {
      ...
      } else courseFragment.updateContent(course, fm);
    } else {
      // One pane layout
      courseFragment = new CourseFragment(course, fm);
  ...
  public void initialize() {
    ...
    if (fragmentContainer != null) {
      ...
      fm.beginTransaction()
          .replace(R.id.fragment_container, timetableFragment, TIMETABLE_FRAGMENT_TAG)
          .commit();
    } else {
      // Two pane layout
    ...
    if (intent == null || intent.getAction() == null) return; // App started normal
