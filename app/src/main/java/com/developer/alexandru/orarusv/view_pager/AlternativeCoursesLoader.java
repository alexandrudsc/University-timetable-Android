...
/**
 * Created by alexandru on 9/16/16. Async task downloader for alternative courses (same course, same
 * prof, different timimg)
 */
public class AlternativeCoursesLoader extends AsyncTask<Void, Void, Void> {
  ...
  protected Void doInBackground(Void... voids) {
    try {
      ...
      AlternativeCoursesParser parser =
          new AlternativeCoursesParser(this.adapter, br, courseToReplace);
      parser.parse();
      conn.disconnect();
