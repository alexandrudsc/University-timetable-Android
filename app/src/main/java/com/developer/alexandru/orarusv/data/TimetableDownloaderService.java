...
/**
 * Created by Alexandru on 2/9/2015. A service downloading the data necessary to a specific
 * timetable. It puts the data in a temporary table of the database and if everything it's ok, it
 * replaces the old data with the fresh one.
 */
public class TimetableDownloaderService extends IntentService {
      ...
      if (parser.wasSuccessful()) {
        Timetable timetable =
            Timetable.Creator.INSTANCE.create(
                new String[] {
                  String.valueOf(timetableType), String.valueOf(timetableID), timetableName
                });
   ...
   *     it's kept for the next version of the app (v1.1.0) where an error code will be used to
    ...
    for (int i = 0; i < time.length; i++) {
      ...
      if (dates.length > 1) {
        if (i == time.length - 2)
          ...
          if (dates.length > 2
              && dates[2] != null
              && dates[2].toLowerCase().contains("deschidere")) {
  ...
  private static class Semester {
    ...
    public static boolean save(Context context, Semester semester) {
      ...
      for (int i = 0; i < numberOfHolidays; i++)
        editor.putString(
            MainActivity.HOLIDAY + "_" + i, semester.startHoliday + "-" + semester.endHoliday);
      editor.commit();
      return true;
    }
  }
