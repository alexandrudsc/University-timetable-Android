...
/** Created by Alexandru on 6/16/14. Adapter for the list with courses in each DayFragment */
public class DayListViewAdapter extends BaseAdapter {
  ...
  public DayListViewAdapter(
      TimetableFragment.OnCourseSelected onCourseSelected, ArrayList<Course> values) {
  ...
  public View getView(int position, View convertView, ViewGroup parent) {
    ...
    boolean currentCourseProgress =
        getCourseProgress(
            courseViewHolder.eventCheckBox.getContext(), courseViewHolder.eventCheckBox.getTag());
  ...
  public static class CourseViewHolder {
    ...
    /**
     * Used when the course view holder is drawed on the screen only with the weekday name. For
     * example the full week view on a single page also includes days name.
     */
     ...
     *
     * @param view - the view on the screen which must hold the courses color, the name and the
     *     description together with the checkbox which marks this course as attended to, or not
     */
    ...
    /**
     * Populates the data into the view and also sets the handlers for click and long click actions
     *
     * @param view the graphical view on the screen
     * @param onCourseSelected - the handler for when the visual element is selected
     * @param fileName - the file on the disk which reminds me if this course was as attended to, or
     *     not
     * @param c - the course structured data
     */
    public void populate(
        View view, TimetableFragment.OnCourseSelected onCourseSelected, String fileName, Course c) {
  ...
  private boolean getCourseProgress(Context context, Object tag) {
    ...
    final SharedPreferences currentWeekProgressFile =
        context.getSharedPreferences(fileAndPreference[0], Context.MODE_PRIVATE);
