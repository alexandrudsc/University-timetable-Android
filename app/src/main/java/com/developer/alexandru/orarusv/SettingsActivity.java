...
/**
 * Created by Alexandru on 7/30/14. Download page.Gets strings to form url then starts AsyncTask.
...
public class SettingsActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    faculties = findViewById(R.id.spinner_faculty);
    groupUndergraduates = findViewById(R.id.spinner_group_undergraduate);
    groupMasters = findViewById(R.id.spinner_group_master);
    groupPhd = findViewById(R.id.spinner_group_phd);
    ...
    groupUndergrAdapter =
        new ArrayAdapter<Elem>(this, R.layout.spinner_item_layout, groupsUndergraduates);
    ...
    // register event receiver for notification of the end of the syncing operation (only if it was
    // successful)
  ...
  private void resetGroupsSpinners() {
    ...
    if (groupMasters == null) groupMasters = findViewById(R.id.spinner_group_master);
    groupMasters.setVisibility(View.INVISIBLE);
    if (groupPhd == null) groupPhd = findViewById(R.id.spinner_group_phd);
    groupPhd.setVisibility(View.INVISIBLE);

    if (prefs == null) prefs = getPreferences(MODE_PRIVATE);
      ...
      switch (view.getId()) {
        case R.id.download_btn:
          ...
          if (facultyID != 0) { // Check if non-modular timetable is requested
            ...
            //                        downloaderTask = new
            // TimetableDownloaderTask(SettingsActivity.this);
    ...
    protected void onPostExecute(Void aVoid) {
      ...
      switch (type) {
        case SQLStmtHelper.UNDERGRADUATES:
          if (studiesLevel.equals(LICENCE)) groupUndergraduates.setSelection(positionToChoose);
          break;
        case SQLStmtHelper.MASTERS:
          if (studiesLevel.equals(MASTERS)) groupMasters.setSelection(positionToChoose);
          break;
        case SQLStmtHelper.PHD:
          if (studiesLevel.equalsIgnoreCase(PHD)) groupPhd.setSelection(positionToChoose);
          break;
      }
    }
  }
