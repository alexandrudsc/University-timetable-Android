...
/** Created by Alexandru on 7/4/14. NOT USED! */
@Deprecated
public class SearchableActivity extends FragmentActivity { // extends ListActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    } else if (intent.getAction().equals(Intent.ACTION_VIEW) && bundle != null) {
      info = bundle.getString(SearchManager.EXTRA_DATA_KEY);
      typeAndName = intent.getData().toString().split("/");
      name = typeAndName[1];
      type = typeAndName[0];

    } else if (intent.getAction().equals(CourseFragment.actionViewDetails)) {
    ...
    linearLayout.addView(
        new CourseFragment(course, getSupportFragmentManager())
            .onCreateView(
                (LayoutInflater) getSystemService(LAYOUT_INFLATER_SERVICE), linearLayout, null));
  ...
  public boolean onCreateOptionsMenu(Menu menu) {
    ...
    inflater.inflate(R.menu.searchable_fragment, menu);
    return true;
  }
